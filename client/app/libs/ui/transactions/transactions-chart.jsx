...
import { useAnalytics } from "@/app/dashboard/api/transactions/get-transaction-analytics";
import { formatAmount, formatAmountShort } from "../../utils/utils";
...
export function TransactionChart() {
  const { analytics, isLoading, isRefetching, } = useAnalytics();
  ...
  const hasNoData =
    ...
    analytics?.weekly_data?.every(
      (item) => !item?.deposit && !item?.transfer && !item?.withdraw
    );

  const data = {
    labels:  analytics?.weekly_data?.map((data) => data?.week),
    datasets: [
      {
        label: analytics?.labels?.[0] || "Deposit",
        data: analytics?.weekly_data?.map((data) => data?.deposit || 0),
        backgroundColor: "#10B981",
        borderRadius: 6,
        borderSkipped: false,
      },
      {
        label: analytics?.labels?.[1] || "Transfer",
        data: analytics?.weekly_data?.map((data) => data?.transfer || 0),
        backgroundColor: "#03457C",
        borderRadius: 6,
        borderSkipped: false,
      },
      {
        label: analytics?.labels?.[2] || "Withdraw",
        data: analytics?.weekly_data?.map((data) => data?.withdraw || 0),
  ...
  return (
    <main className="w-full bg-white rounded-xl border border-gray-200 p-4 sm:p-7 mb-10">
      <section className="flex flex-col gap-1 pb-2">
        ...
        >
          <p>
            In: <b className="text-emerald-600">{formatAmount(analytics?.totalIn ?? 0)}</b>
          </p>
          <p>
            Out: <b className="text-rose-600">{formatAmount(analytics?.totalOut || 0)}</b>
          </p>
        </div>
      </section>
