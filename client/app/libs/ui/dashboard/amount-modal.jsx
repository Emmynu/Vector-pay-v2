...
function AmountModal({ id }) {
  ...
  return (
    <dialog id={id} className="modal backdrop-blur-sm">
      ...
      <button 
      className="rounded-full p-1.5 cursor-pointer text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors outline-none border-none" disabled={isDepositing} onClick={closeModal}>
        <X className="w-4"/>
      </button>
        </div>
        ...
        <form  onSubmit={handleDeposit}>
          <div>
            ...
            {(amount && Number(amount) < 10) && (
                <p className="text-[11px] text-rose-600 mt-1 font-medium" style={quicksand.style}>
