...
function DashboardHeader({ setOpen, user, isLoading }) {
    ...
    return ( 
         <header className="h-16 sticky top-0 z-20 backdrop-blur-xl border-b border-slate-200 flex items-center justify-between px-4 lg:px-8">
          ...
          <div className="flex items-center gap-0">
            ...
            <section className="menu menu-horizontal px-0 ">
              <li className="dropdown dropdown-center ">
                ...
                <ul tabIndex="-1" className="dropdown-content border border-slate-200 bg-white rounded-box z-1 w-52 p-2 shadow-sm">
                  ...
                  <li><Link href={"/dashboard/profile"} className="flex items-center gap-1 hover:opacity:80  mt-1"><User2 className="w-3.5 h-3.5"/> <h2 className={`${quicksand.className} tracking-wide font-medium text-xs md:text-[13px]`}>Profile</h2></Link></li>

                  <li>
                   <button  onClick={handleResetPassword} className={`${quicksand.className} tracking-wide font-medium text-xs md:text-[13px] hover:opacity-80 disabled:opacity-80 my-1`} disabled={isResetting || isLogginOut}>
                      {isResetting ? (
                  ...
                  <li>
                    <button  onClick={logout} className={`${quicksand.className} tracking-wide font-medium text-xs md:text-[13px] hover:opacity-80 disabled:opacity-80`} disabled={isLogginOut || isResetting}>
                      {isLogginOut ? (
                        <div className="flex items-center">
                          <span className="loading loading-spinner loading-xs mr-1"></span>
                          <h2>Signing out</h2>
            ...
            {isLoading ? 
              ...
              :
             <span className="inline-flex items-center px-2.5 py-1 mt-1 rounded-full bg-[#E6F0FA] text-[#4A90E2] text-xs font-semibold">
              {`Tier ${user?.tier || "1"}`}
            </span>
            }
